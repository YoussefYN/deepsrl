from deepsrl.neural_srl.shared import features
...
def get_pretrained_embeddings(filepath, embeddings_dir=""):
    embeddings = dict()
    with open(os.path.join(embeddings_dir, filepath), 'r') as f:
        for line in f:
...
def get_postag_test_data(filepath, config, word_dict, label_dict, embeddings_dir="", allow_new_words=True):
    ...
    if filepath != None and filepath != '':
        ...
        samples = []
    word_to_embeddings = get_pretrained_embeddings(WORD_EMBEDDINGS[config.word_embedding], embeddings_dir)
    if allow_new_words:
...
def get_srl_test_data(filepath, config, word_dict, label_dict, embeddings_dir="", allow_new_words=True):
    word_dict.accept_new = allow_new_words
    if label_dict.accept_new:
        label_dict.set_unknown_token(UNKNOWN_LABEL)
        label_dict.accept_new = False
